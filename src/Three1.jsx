...
export const Three1 = () => (
    <Canvas flat camera={{ fov: 100, position: [0, 0, 20] }} eventSource={document.getElementById('root')} eventPrefix="client">
        <color attach="background" args={['#94e1ff']} />
        <Frame id="01" name="Login" author="Haz doble Click" bg="#e4cdac" position={[-1.15, 0, 0]} rotation={[0, 0.5, 0]}>
            <Gltf src="pickles_3d_version_of_hyuna_lees_illustration-transformed.glb" scale={8} position={[0, -0.7, -2]} />
        </Frame>
        <Frame id="02" name={`CTVWEB\n2025`}  author="Haz doble Click">
            <Gltf src="fiesta_tea-transformed.glb" position={[0, -2, -3]} />
        </Frame>
        <Frame id="03" name="Acerca" author="Haz doble Click" bg="#d1d1ca" position={[1.15, 0, 0]} rotation={[0, -0.5, 0]}>
            <Gltf src="House_001_GLB.glb" scale={2} position={[0, -0.8, -4]} />
        </Frame>
        <Rig />
        <Preload all />
    </Canvas>
...
function Frame({ id, name, author, bg, width = 1, height = 1.61803398875, children, ...props }) {
    ...
    return (
        <group {...props}>
            <Text font={suspend(medium).default} fontSize={0.1} anchorY="top" anchorX="left" lineHeight={0.8} position={[-0.375, 0.715, 0.01]} material-toneMapped={false}>
                {name}
            </Text>
